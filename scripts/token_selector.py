"""
token_selector.py — Select best portfolio tokens from CoinGecko
based on 24h performance. Only includes tokens traded on Hyperliquid.

Usage:
  python3 token_selector.py --top 5 --direction bullish
  python3 token_selector.py --top 5 --direction bearish
"""
import argparse, requests

# All tokens available on Hyperliquid with their CoinGecko IDs
HL_TOKENS = {
...
if __name__ == '__main__':
    ...
    bearish = args.direction == 'bearish'
    tokens = get_top_tokens(args.top, bearish)
    label = 'Top Gainers' if not bearish else 'Worst Performers'
    print(f"\n{label} (Hyperliquid-traded, top {args.top}):\n")
    print(f"  {'Symbol':<8} {'Price':>10} {'24h %':>8}")
    print("  " + "-" * 30)
    for t in tokens:
        print(f"  {t['symbol']:<8} ${t['price']:>9,.4f} {t['pct_24h']:>+7.2f}%")
